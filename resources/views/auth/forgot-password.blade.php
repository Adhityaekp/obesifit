...
<body class="font-poppins bg-light">
    ...
    <main class="min-h-screen flex items-center justify-center py-12 px-4">
        ...
        <div class="w-full max-w-md bg-white rounded-2xl shadow-2xl overflow-hidden p-8">
            <div class="text-center mb-8">
                <h1 class="text-3xl font-bold text-dark mb-2">Lupa Kata Sandi</h1>
                <p class="text-gray-600">Masukkan email Anda untuk menerima tautan reset kata sandi.</p>
            </div>
            ...
            <form method="POST" action="{{ route('password.email') }}" class="space-y-6">
                ...
                <div class="text-center">
                    <p class="text-gray-600">Ingat kata sandi Anda?
    ...
    <script>
        // Menghilangkan pesan error/sukses setelah beberapa detik
    ...
    <script>
        // Alert untuk sukses mengirim email reset password
        document.addEventListener('DOMContentLoaded', function() {
            @if (session('status'))
                alert("{{ session('status') }}. Silakan cek email Anda.");
            @endif
